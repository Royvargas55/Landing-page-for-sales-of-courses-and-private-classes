$(document).ready(function() {
	...
	jQuery("#callActionForm").validator().on("submit", function (event) {
		...
		if (event.isDefaultPrevented()) {
			...
			formError();
			submitMSG(false, "Porfavor verifique haber llenado todos los campos");
		} else {
	...
	function formError(){
		...
		$("#callActionForm").removeClass().addClass('shake animated').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
			$(this).removeClass();
		});
	}
